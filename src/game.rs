...
impl Game {
    ...
    pub fn _get_buckets(&self) -> (HashMap<u8, Pet>, &Vec<Food>) {
    ...
    pub fn _bot_random(&mut self) {
        self.roll_shop(0).unwrap();
        for x in 0..3 {
            self.buy_pet(0, x).unwrap();
        }
    }
    ...
    pub fn _battle(&mut self, mut enemy_crew: Crew) -> u8 {
        ...
        loop {
            ...
            match _check_win_condition(&my_crew, &enemy_crew) {
                3 => {}
                x => return x,
            }
            ...
            _headon_attack(my_attacker, enemy_attacker);
            _headon_attack(enemy_attacker, my_attacker);

            if _check_life(my_attacker) {
                my_index += 1;
            }
            if _check_life(enemy_attacker) {
                enemy_index += 1;
            }
            //break;
        }
    ...
    pub fn _game_loop(&mut self, bot: Crew) {
        ...
        loop {
            ...
            loop {
                ...
                let x = util::_wait_for_input();

                match x {
                    // buy mode
                    1 => {
                        println!("What shop pet do you want?");
                        let shop_pet: u8 = util::_wait_for_input() - 1;
                        println!("What team slot do you want to put that pet?");
                        let team_slot: u8 = util::_wait_for_input() - 1;
                        self.buy_pet(shop_pet, team_slot).unwrap();
                    }
                    // swap mode
                    2 => {
                        println!("Insert position of pet one");
                        let pet_one: u8 = util::_wait_for_input() - 1;
                        println!("Insert position of pet two");
                        let pet_two: u8 = util::_wait_for_input() - 1;
                        self.swap_pet(pet_one as usize, pet_two as usize).unwrap();
                    }
                    // Roll shop
                    3 => {
                        self.roll_shop(1).unwrap();
                    }
                    // Sell pet
                    4 => {
                        println!("What pet do you want to sell?");
                        let pet = util::_wait_for_input() - 1;
                        self.crew.sell_pet(pet as usize).unwrap();
                    }
                    // Freeze pet
                    5 => {
                        println!("What pet do you want to freeze/unfreeze?");
                        let pet = util::_wait_for_input() - 1;
                        self.store.freeze_and_unfreeze_pet(pet as usize).unwrap();
                    }
                    // buy food
                    6 => {
                        ...
                        let team_slot: u8 = util::_wait_for_input() - 1;
                        self.buy_food(shop_pet, team_slot).unwrap();
                    }
                    // freeze food
                    7 => {
                        println!("What food do you want to freeze/unfreeze?");
                        let food = util::_wait_for_input() - 1;
                        self.store.freeze_and_unfreeze_food(food as usize).unwrap();
                    }
                    // end turn mode
                    99 => break,
                    _ => {}
